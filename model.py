...
class ASPestNet(nn.Module):
    def __init__(self, delay_lengths, ir_length=1.8):
        ...
        self.fC1ProjLayer = ProjectionLayer(
            (self.enc_outp_shape[0], self.enc_outp_shape[1]), 1, 8, 
            bias = bias_f(omegaK),
        ...
        self.mC1ProjLayer = ProjectionLayer(
            (self.enc_outp_shape[0], self.enc_outp_shape[1]), 3, 8,
            bias = bias)
        ...
        # delay lengths, das hier könnten wir mit unseren delay lengths ersetzen, brauchen dann aber noch dAP
        self.d = torch.tensor([233, 311, 421, 461, 587, 613],  device=get_device())
        self.M = self.d.size(0)

        self.SAProjLayer = ProjectionLayer(
            (self.enc_outp_shape[0], self.enc_outp_shape[1]), self.M, 4, 
            activation = self.sigmoid)
        self.bcProjLayer = ProjectionLayer(
            (self.enc_outp_shape[0], self.enc_outp_shape[1]), 2, self.M)
        self.hProjLayer = ProjectionLayer(
            (self.enc_outp_shape[0], self.enc_outp_shape[1]), 1, 232)

        # self.Q0 = torch.tensor(householder(self.M), device=get_device())  # TODO: when to change Houlseholder matrix ? 
        self.Q0 = 1/3 * torch.ones(self.M,self.M, device=get_device()) - torch.eye(self.M, device=get_device())
        ...
        # length of IR  
        self.ir_length = int(self.ir_length*self.sr)
